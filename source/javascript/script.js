 function painter(canvas) {
      canvas.height = Math.min(500, window.innerHeight-(200 + window.innerHeight % 20));
    ...
    painter.prototype.logPolar = function(data) {
      ...
      for(var i = 0; i < canvas.height; i += 1){
        for(var j = 0; j < canvas.width; j += 1){
          ...
          var xScale = (canvas.width - 1) / Math.log(Math.sqrt(Math.pow(canvas.height,2) + Math.pow(canvas.width,2))/2)
